...
class _PetDetailPageState extends State<PetDetailPage> {
  ...
  Widget build(BuildContext context) {
    ...
    final profile = _pet!['profiles'] as Map<String, dynamic>?;
    final ownerName = profile?['display_name'] as String? ?? 'Desconocido';
          ...
          children: [
            if (_photos.isNotEmpty)
              ...
              Container(
                height: 300,
                color: AppColors.blue.withOpacity(0.15),
                child: const Center(child: Icon(Icons.pets, size: 80, color: AppColors.navy)),
              ),
            Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Card(
                    child: Padding(
                      ...
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text('Detalles', style: Theme.of(context).textTheme.titleLarge!.copyWith(fontWeight: FontWeight.w700)),
                          const SizedBox(height: 12),
                          _DetailRow(icon: Icons.pets, label: 'Especie', value: especie == 'perro' ? 'Perro' : 'Gato'),
                          if (raza != null) _DetailRow(icon: Icons.category, label: 'Raza', value: raza),
                          if (edadMeses != null) _DetailRow(icon: Icons.cake, label: 'Edad', value: _formatEdad(edadMeses)),
                          if (talla != null) _DetailRow(icon: Icons.straighten, label: 'Talla', value: talla),
                          if (sexo != null) _DetailRow(icon: Icons.wc, label: 'Sexo', value: sexo == 'macho' ? 'Macho' : 'Hembra'),
                          if (temperamento != null) _DetailRow(icon: Icons.emoji_emotions, label: 'Temperamento', value: temperamento),
                        ],
                      ),
                    ),
                  ),
                  ...
                  if (descripcion != null && descripcion.isNotEmpty) ...[
                    Text('Descripción', style: Theme.of(context).textTheme.titleLarge!.copyWith(fontWeight: FontWeight.w700)),
                    const SizedBox(height: 8),
                    Card(
                      child: Padding(
                        padding: const EdgeInsets.all(16),
                        child: Text(descripcion, style: Theme.of(context).textTheme.bodyLarge),
                      ),
                    ),
                    const SizedBox(height: 16),
                  ],

                  Text('Publicado por', style: Theme.of(context).textTheme.titleLarge!.copyWith(fontWeight: FontWeight.w700)),
                  const SizedBox(height: 8),
                  Card(
                    child: ListTile(
                      leading: CircleAvatar(child: Text(ownerName[0].toUpperCase())),
  ...
  String _formatEdad(int meses) {
    if (meses < 12) {
      ...
      final anios = meses ~/ 12;               // <- sin 'ñ' en el nombre del identificador
  ...
  Color _getEstadoColor(String estado) {
    switch (estado) {
      case 'publicado': return AppColors.blue.withOpacity(0.2);
      case 'adoptado':  return Colors.green.withOpacity(0.2);
      case 'reservado': return Colors.orange.withOpacity(0.2);
